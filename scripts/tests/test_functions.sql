...
PRINT 'tvf USD base/tax: ' + CASE WHEN EXISTS
      (SELECT 1 FROM @r WHERE BaseUnitPrice = 4.99 AND TaxRatePct = 9.00) THEN 'PASS' ELSE 'FAIL' END;
...
-- Technical Design Document §4.1; Mappings and Formula §5.1: a missing rate must raise the
-- "currency rate not found" code (CUR006), not silently price at 1.0.
DECLARE @r TABLE (BaseUnitPrice DECIMAL(19,4), PromotionPct DECIMAL(9,4), EffectiveUnitPrice DECIMAL(19,4),
                  NetBeforeTax DECIMAL(19,4), TaxRatePct DECIMAL(5,2), EstimatedTax DECIMAL(19,4),
                  ExtendedAmount DECIMAL(19,4), PricingNote NVARCHAR(200));
INSERT INTO @r
...
PRINT 'tvf missing-currency CUR006: ' + CASE WHEN EXISTS
      (SELECT 1 FROM @r WHERE PricingNote LIKE '%CUR006%') THEN 'PASS' ELSE 'FAIL' END;
GO
